...
namespace Tassk05
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 1; i <= ((factorielDividend - factorielDivisor)); i++)
            {
                diffFacturiel = i * factorielDivisor;
            }
            Console.WriteLine("N! =  " + factorielDividend);
            Console.WriteLine("K! = " + factorielDivisor);
            Console.WriteLine("(N-K)! = " + diffFacturiel);
            finalResult = Convert.ToDouble(factorielDividend * factorielDivisor) / Convert.ToDouble(diffFacturiel);
